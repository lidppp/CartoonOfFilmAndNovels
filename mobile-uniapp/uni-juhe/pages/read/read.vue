...
<script>
	...
	import { detailsXs, contentXs } from "../../api/api.js"
	...
	export default {
		...
		onLoad(option) {
			this.chapterIndex = parseInt(option.index); // 当前章节
			this.bookId = option.url1; // url1目录
			this.getSystemInfo();
			this.history.chapterIndex = this.chapterIndex
		},
		watch:{
			"curChapter":{
				handler(){
		...
		methods: {
			...
			**/
			getDirectoryList(showLoading) {
				...
				return detailsXs(this.bookId).then(([err,data])=>{
					const res = data.data
					if(res.code != 0 || err){
						uni.hideLoading()
						return new Promise((resolve, reject)=> {
					...
					for (let i=0;i<res.list.length;i++) {
						this.directoryList.push({
							index: i,
							chapterId: res.list[i].url,   //注意：这个chapterId用于获取章节内容而不是index
							name: res.list[i].num
						})
					}
			...
			**/
			getOneChapter(chapterId, showLoading) {
				...
				return contentXs(chapterId).then(([err,data])=>{
					const res = data.data
					if (showLoading) {
						uni.hideLoading()
					}
					...
					for(let i = 0 ; i < res.content.length; i++){
						this.tmpChapter.text +=  `<p>${res.content[i]}</p>`
			...
			**/
			...
			HistoryStorage(){
				...
				storage.SetStorage("xsHistory",{
					coverImg:this.coverImg,
					lastNum:this.curChapter.chapterName,
					name:this.bookName,
					url1:this.bookId,
					index:this.curChapter.chapterIndex
				})
			}
